...
export default function ArticleDetailScreen() {
  ...
  const selectedArticle = useSelectedArticleStore((state) => state.selectedArticle);
  const toggleBookmark = useBookmarksStore((state) => state.toggleBookmark);
  const bookmarks = useBookmarksStore((state) => state.bookmarks);
  const [article, setArticle] = useState<Article | null>(
    selectedArticle && selectedArticle.id === articleId ? selectedArticle : null
  );
  ...
  if (!article) {
    return <FeedState title="Article unavailable" message="Unable to open this article." />;
  }

  const isBookmarked = Boolean(bookmarks[article.id]);
  const formattedDate = article.time ? format(new Date(article.time * 1000), 'MMM d, yyyy p') : '';
  const headerTitle = article.title.length > 36 ? `${article.title.slice(0, 36)}...` : article.title;
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {article.url ? (
          <View style={styles.webviewContainer}>
            <WebView nestedScrollEnabled source={{ uri: article.url }} style={styles.webview} />
          </View>
        ) : (
